...
function _showImportDiff(d) {
  ...
  const incomingSess = d.ais.reduce((a, x) => a + (x.sessions ? x.sessions.length : 0), 0);
  ...
  let sessToAdd = 0;
  existingAIs.forEach(incoming => {
    const cur = currentAIs.find(a => a.name.toLowerCase() === incoming.name.toLowerCase());
    if (!cur) return;
    const curIds = new Set(cur.sessions.map(s => s.id));
    sessToAdd += (incoming.sessions || []).filter(s => !curIds.has(s.id)).length;
  });
  newAIs.forEach(a => { sessToAdd += (a.sessions || []).length; });
  ...
  ch.innerHTML = `
    ...
    <div class="import-diff-row"><span class="import-diff-label">IAs nuevas</span><span class="import-diff-val new">+${newAIs.length}</span></div>
...
function confirmImport() {
  ...
  const prevAIs = state.ais ? state.ais.length : 0;
  state = d; save(); render(); applyTheme(state.theme || 'dark');
  const newAIs = state.ais.length;
  const newSess = state.ais.reduce((a, x) => a + x.sessions.length, 0);
  closeImportDiff();
  if (newAIs === 0 && newSess === 0) { showToast('warning', 'Archivo importado — sin datos nuevos'); return; }
  showToast('success', `Importado: ${newAIs} IA${newAIs !== 1 ? 's' : ''} y ${newSess} sesión${newSess !== 1 ? 'es' : ''}`);
}
