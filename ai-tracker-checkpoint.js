...
function showCheckpointPanel(result) {
      ...
      if (!applyError && window._pendingTemplateDownload) {
        ...
        if (_templateTrigger() === 'session') {
          if (typeof downloadTemplates === 'function') {
            ...
            if (typeof showToast === 'function') showToast('warning', '⚠️ Descarga no disponible — módulo de plantillas no cargado');
          }
        }
      }
    };
  ...
  if (!sections.length) {
    if (typeof showToast === 'function') showToast('info', 'Este CHECKPOINT no tiene ítems ni próximo paso — nada que mostrar.');
    return;
  }
...
// Toast stack system — extraído a locus-toast.js (R-202605-003)
// showToast, showToastDigest, showToastInline, _toastRender, _dismissToast, _toastNext, toast
// locus-toast.js debe cargarse antes que este módulo.
